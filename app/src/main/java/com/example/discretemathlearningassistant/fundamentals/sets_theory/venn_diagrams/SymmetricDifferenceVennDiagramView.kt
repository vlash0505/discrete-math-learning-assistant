...
class SymmetricDifferenceVennDiagramView(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val differencePathA = Path().apply {
            op(pathA, pathB, Path.Op.DIFFERENCE)
        }
        val differencePathB = Path().apply {
            op(pathB, pathA, Path.Op.DIFFERENCE)
        }
        val symmetricDifferencePath = Path().apply {
            op(differencePathA, differencePathB, Path.Op.UNION)
        }

        circlePaint.color = Color.argb(127, 255, 127, 0)
        canvas.drawPath(symmetricDifferencePath, circlePaint)
