...
class BFSGraphTraversalView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    private val nodeCount = 7
    private val nodes = List(nodeCount) { Node() }
    private val edges = mutableListOf<Edge>()
    private val bfsQueue: Queue<Node> = LinkedList()
    ...
    private val bfsPaint = Paint().apply {
        color = Color.RED
        strokeWidth = 4f
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (node in nodes) {
            ...
            val paint = when {
                ...
                visitedNodes.contains(node) -> bfsPaint // Draw visited nodes in red
        ...
        if (bfsQueue.isNotEmpty()) {
            val node = bfsQueue.remove()
            visitedNodes.add(node)

            for (edge in edges) {
                if (edge.node1 == node && !visitedNodes.contains(edge.node2) && !bfsQueue.contains(edge.node2)) {
                    bfsQueue.add(edge.node2)
                } else if (edge.node2 == node && !visitedNodes.contains(edge.node1) && !bfsQueue.contains(edge.node1)) {
                    bfsQueue.add(edge.node1)
                }
            }
