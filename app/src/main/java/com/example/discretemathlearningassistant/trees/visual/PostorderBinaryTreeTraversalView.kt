...
class PostorderBinaryTreeTraversalView @JvmOverloads constructor(
    context: Context,
    ...
    private val postorderTraversal: List<TreeNode> = listOf(
        tree!!.left!!.left!!,
        tree!!.left!!.right!!,
        tree!!.left!!,
        tree!!.right!!.left!!,
    ...
    init {
        handler.post(object : Runnable {
            override fun run() {
                if (stepIndex < postorderTraversal.size) {
                    visitedNodes.add(postorderTraversal[stepIndex])
                    stepIndex++
                    invalidate()
