...
class PreorderBinaryTreeTraversalView @JvmOverloads constructor(
    context: Context,
    ...
    private val preorderTraversal: List<TreeNode> = listOf(
        tree!!,
        tree!!.left!!,
        tree!!.left!!.left!!,
        tree!!.left!!.right!!,
    ...
    init {
        handler.post(object : Runnable {
            override fun run() {
                if (stepIndex < preorderTraversal.size) {
                    visitedNodes.add(preorderTraversal[stepIndex])
                    stepIndex++
                    invalidate()
